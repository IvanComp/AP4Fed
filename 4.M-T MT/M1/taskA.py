...
class NYUv2SegDataset(Dataset):
    def __init__(self, data_dir="./data", transform_rgb=None, transform_label=None):
        ...
        self.label_files = sorted([f for f in os.listdir(self.labels_dir) if not f.startswith('.')])
...
def load_data():
    transform_img = Compose([
        Resize((480, 640), interpolation=Image.NEAREST),
        ToTensor(),
        Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
    ])
    transform_lbl = Resize((480, 640), interpolation=Image.NEAREST)
    ...
    # Riduciamo il batch size per contenere l'uso della memoria
...
def train(net, trainloader, valloader, epochs, device):
    ...
    for epoch in range(epochs):
        for images, labels in trainloader:
            ...
            # Liberiamo la memoria relativa al batch
    ...
    # Spostiamo il modello sulla CPU prima del testing e dell'invio dei pesi
    net.to("cpu")
    ...
    # Otteniamo i pesi dopo che il modello è stato spostato sulla CPU
    weights = get_weights(net)
    
    return results, training_time, weights
...
def test(net, testloader):
    ...
    correct_pixels = 0
    total_pixels = 0
    true_positive = torch.zeros(40)
    false_positive = torch.zeros(40)
    false_negative = torch.zeros(40)
    ...
    with torch.no_grad():
        for images, labels in testloader:
            ...
            del images, labels, outputs, preds, tp, fp, fn
            torch.cuda.empty_cache()
            gc.collect()
    ...
    f1_val = f1_per_class.mean().item()

    del true_positive, false_positive, false_negative, precision, recall, f1_per_class
    torch.cuda.empty_cache()
    gc.collect()
...
def f1_score_torch(y_true, y_pred, num_classes, average='macro'):
    ...
    precision = torch.zeros(num_classes, device="cpu")
    recall = torch.zeros(num_classes, device="cpu")
    f1_per_class = torch.zeros(num_classes, device="cpu")
...
def get_weights(net):
    # Assicuriamoci di non accumulare gradiente e di operare sulla CPU
    with torch.no_grad():
        state_dict = net.state_dict()
        weights = [state_dict[k].cpu().numpy() for k in state_dict.keys()]
    return weights
