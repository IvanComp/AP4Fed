...
# Definizione del device e stampa di controllo
DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
#log(INFO, f"Device Used - taskA: {DEVICE}")
...
# CLIENT
global DATASET_TYPE, DATASET_NAME, CUSTOM_MODEL
DATASET_TYPE = ""
DATASET_NAME = "CIFAR10"  # default
CUSTOM_MODEL = None  # Global for user-defined model
...
class Net(nn.Module):
    def __init__(self) -> None:
        ...
        # Usa il modello personalizzato se definito
        global CUSTOM_MODEL
        if CUSTOM_MODEL is not None:
            self.model = CUSTOM_MODEL
        else:
            self.densenet121 = models.densenet121(weights=None, num_classes=AVAILABLE_DATASETS[DATASET_NAME]["num_classes"])
        self.net = self.densenet121 if CUSTOM_MODEL is None else self.model
...
def load_data():
    ...
    if HETEROGENEOUS_DATA_HANDLER:
        ...
        if DATASET_TYPE == "IID":
            ...
            return DataLoader(trainset, batch_size=64, shuffle=True, num_workers=1), DataLoader(testset)
        ...
        if DATASET_TYPE == "non-IID":
            ...
            trainloader = DataLoader(subset_augmented, batch_size=32, shuffle=True, num_workers=1)
...
def train(net, trainloader, valloader, epochs, DEVICE):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(trainloader):
            if batch_idx % 10 == 0:
                log(INFO, f"Epoch {epoch+1} - Processing batch {batch_idx+1}/{len(trainloader)}")
        ...
        log(INFO, f"Epoch {epoch+1} finished in {epoch_duration:.2f} seconds.")
    training_time = time.time() - start_time
    log(INFO, f"Training completed in {training_time:.2f} seconds")
    start_comm_time = time.time()
    train_loss, train_acc, train_f1, train_mae = test(net, valloader)  # In test, il modello è messo in eval()
...
def test(net, testloader):
    net.eval()  # Imposta il modello in modalità evaluation
    net.to(DEVICE)
